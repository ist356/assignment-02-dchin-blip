...
def parse_packaging(packaging_data: str) -> list[dict]:
    ...
    for specification in items:
        division = specification.split(" in ")
        pair = division[0].split()
        quantity = int(pair[0])
        item = pair[1]
        packaging.append({item:quantity})
    item = specification.split(" in ")[-1]
...
def calc_total_units(package: list[dict]) -> int:
    ...
    '''
    count = 1
    for specification in package:
        count *= list(specification.values())[0]
    return count
